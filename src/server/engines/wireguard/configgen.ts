...
import { removeNewlines } from '../../utils/template';
...
function shouldIncludeAmneziaParams(executable?: string): boolean {
  if (executable !== undefined) {
    return executable === 'awg';
  }
  return typeof WG_ENV !== 'undefined' && WG_ENV.WG_EXECUTABLE === 'awg';
}
...
AllowedIPs = ${allowedIps.join(', ')}${extraLines.length ? `\n${extraLines.join('\n')}` : ''}`;
  ...
  generateServerInterface: (
    ...
    if (shouldIncludeAmneziaParams(options.executable)) {
      const parameters = {
        Jc: wgInterface.jC,
