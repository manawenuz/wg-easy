...
> **2026-05-18 (later) — UAT on Hetzner cx23 passed, with bugs caught.** Provisioned `wgeasy-quota-uat` (cx23, Debian 12), deployed `sha-5456d94` with AmneziaWG compose, populated fixtures via GUI, built new image on the VM, upgraded in place, drove 600 MB across 3 docker WG clients. Family bucket tripped at 671 MB / 500 MB; all 3 manwe peers auto-disabled; `family.quota.exceeded` audit event with the correct `disabledClientIds`. VM destroyed. Two **critical** migration bugs surfaced and fixed (`0016` missing `--> statement-breakpoint` markers; libsql migrator chokes on leading SQL comments in `0018`). New PRDs filed: **60-17** suppress upstream update banner, **60-18** AmneziaWG interface-edit validation (operator blocker), **60-19** migration authoring guard (CI lint), **20-07** wizard engine picker, **20-08** lifetime quota period. Upstream merge TODO at `handoff/upstream-merge-todo.md`. Full run log in `handoff/uat-quota-family-upgrade.md`.

> **2026-05-18 — PRD-20-06 + PRD-60-13 + PRD-60-14 shipped (then UAT'd).**
> - **`20-user-features/06-quota-unit-picker`** — shipped via Kimi. `QuotaForm.vue` now uses `BaseInput` + `BaseSelect` (MB/GB/TB) with `fromBytes()` back-conversion. No backend changes. Open follow-up: `BaseSelect` renders dropdown items as `MB - MB` (pre-existing component template; out of scope here).
> - **`60-bugfixes/13-per-user-aggregate-quota`** — shipped via Kimi. `quota` table reshaped into `user_quota` (one row per user). `usagePoller` aggregates by `userId`; `quotaEvaluator` disables every peer of an over-limit user atomically + emits a single `user.quota.exceeded` audit event. `PUT/DELETE /api/admin/clients/{id}/quota` now return `410 Gone`. Client creation guards against over-limit users (creates as disabled). Kimi flagged five PRD-gap files (`schema.ts`, `sqlite.ts`, `client/[clientId]/index.get.ts`, `client/index.post.ts`, `admin/users/[id].vue`) — retroactively added to PRD `touches:` + authoring lesson recorded.
> - **`60-bugfixes/14-shared-quota-pool-subaccounts`** — shipped via Kimi same day. Extends 60-13 so sub-accounts share the parent's bucket: bucket key collapses from `user_id` to `root_user_id` (via new `userService.getRootUserId`); migration `0017` deletes orphan sub-account quota rows; new `GET /api/admin/users/{id}/quota-breakdown` endpoint surfaces per-member contribution (admin-visible breakdown only — no sub-caps). Bucket key collapses from `user_id` to `root_user_id` via new `UserService.getRootUserId` (10-hop cycle cap). Migration `0017` deletes orphan sub-account rows. `quotaEvaluator` emits `family.quota.exceeded` with the union of disabled client ids. New `GET /api/admin/users/{id}/quota-breakdown` shows per-member contribution. Kimi flagged three new PRD-gap files (`client/service.ts`, `usageSample/service.ts`, `vitest.config.ts`) — retroactively added. Two small follow-ups deferred to backlog: surface parent name on user GET (cosmetic), and emit a `user.quota.merged_into_family` audit event when a sub-account attach drops their quota row (audit hygiene). Candidates: PRD-60-15 (parent name) and PRD-60-16 (merge audit event).
>
> **Authoring lessons codified (apply to all future PRDs):**
> 1. When a service is renamed, list `src/server/database/schema.ts` (re-exports) and `src/server/database/sqlite.ts` (DI wiring) in `touches:`.
> 2. When a form moves between pages, list the destination page in `touches:`.
> 3. When the body shape of `GET /api/client` changes, also list the single-client variant `src/server/api/client/[clientId]/index.get.ts` and the POST handler `src/server/api/client/index.post.ts` (creation-time guards).
> 4. When `Verification` calls for unit tests, list each new test file path in `touches:` with a `(new)` marker — otherwise Kimi (correctly) treats them as out of scope.
> 5. When a PRD adds a list-style API or scheduler iterator, list every repository service it will read from in `touches:`, including any new multi-id helpers like `getForUsers` / `getForClients`. Also include `src/vitest.config.ts` if the new test files land in a directory not yet in the include glob.
> 6. When a PRD calls for an audit-log event as part of a transactional mutation (e.g., "emit `X.event` when state changes"), state it explicitly in the `## Scope > In` section, not in passing prose — Kimi treats prose mentions as optional. Same with UI copy that requires extra API plumbing (e.g., "show {parent.name}"): if the field isn't already on the relevant GET response, list the API file in `touches:` too.
